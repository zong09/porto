...
- **Top nav** (sticky, bg `#FAF5EC`, border-bottom `#eee3d2`): logo, tabs (ภาพรวม / พอร์ต / Transactions / หนี้สิน) with active = 700 + 2px `#b45a3c` underline, THB/USD pill toggle (active pill `#3d3328`/`#faf5ec`), "+ เพิ่มรายการ" (`#b45a3c`), "ออกจากระบบ" (outline).
...
{ portfolios:[{id,name,color}],
  ...
  fx, lastUpdated, cur:'THB'|'USD', seq }
```
