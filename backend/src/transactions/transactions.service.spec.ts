...
describe('TransactionsService', () => {
      ...
      providers: [
        ...
        },
        {
          ...
          useValue: {
            calculate: jest
              .fn()
              .mockReturnValue({ quantity: 20, avgCost: 100 }),
          },
        },
      ],
    }).compile();
  ...
  describe('create', () => {
    ...
    it('should throw NotFoundException if asset not owned or exists', async () => {
      ...
      await expect(
        service.create('user-1', 'invalid-asset', 'buy', 5, 100, 2, ''),
    ...
    it('should throw BadRequestException if side is invalid for deposit asset', async () => {
      ...
      await expect(
        service.create('user-1', 'asset-2', 'invalid' as any, 500, 1, 0, ''),
      ).rejects.toThrow(BadRequestException);
    });

    it('should throw BadRequestException if selling more than current quantity', async () => {
      jest.spyOn(positionService, 'calculate').mockReturnValue({
        quantity: 2,
        avgCost: 100,
        realizedPnl: 0,
        totalCost: 200,
      ...
      await expect(
        service.create('user-1', 'asset-1', 'sell', 5, 200, 0, ''),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('update', () => {
    ...
    it('should throw BadRequestException on sell update exceeding holdings limit', async () => {
      ...
      await expect(
        service.update('tx-1', 'user-1', 'asset-1', 'sell', 10, 150, 2, ''),
      ).rejects.toThrow(BadRequestException);
    });
  });
